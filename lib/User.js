...
export class User {
  ...
  checkFormat(attribute) {
    ...
    this.data[attribute] = this.data[attribute].charAt(0).toUpperCase() + this.data[attribute].slice(1).toLowerCase()

    // Fix attribute if there is a dash in it
    const dash = this.data[attribute].indexOf('-')
    if (dash !== -1 && dash !== this.data[attribute].length - 1) {
      this.data[attribute] = this.data[attribute].slice(0, dash + 1) 
